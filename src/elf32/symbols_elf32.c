...
static unsigned char elf32_letter_from_section_name(uint64_t flag, const char *name)
{
	unsigned char c = '?';
...
int elf32_symbols(t_m32 *elf, const Elf32_Shdr *section, const Elf32_Sym *symtab, const char *symname, t_nmhandle *printer)
{
	...
	for (unsigned int i = 1; i < section->sh_size / section->sh_entsize; i++) {
		...
		c = (c != '?') ? c : elf32_letter_from_section_name(printer->flag, elf->string_ptr + elf->section[symtab[i].st_shndx].sh_name);
